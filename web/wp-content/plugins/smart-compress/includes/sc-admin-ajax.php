...
function sm_type_callback()
{
    ...
    $sql = "UPDATE " . $sc_table . " SET value = '" . $_POST['value'] . "' WHERE option = 'type'";
    mysqli_query($link, $sql);
    mysqli_close($link);


    // $sc_type = $wpdb->get_results("SELECT * FROM " . $sc_table);
...
function sm_quality_callback()
{
    ...
    $link = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
    $sql = "UPDATE " . $sc_table . " SET value = '" . $_POST['value'] . "' WHERE option = 'quality'";
    mysqli_query($link, $sql);
    mysqli_close($link);
...
function sm_generate_callback()
{
    ...
    $link = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
    $sql = "UPDATE " . $sc_table . " SET value = '" . json_encode($sc_last_generate) . "' WHERE option ='last_generate' ";
    mysqli_query($link, $sql);
    mysqli_close($link);
...
function sm_add_assets_callback()
{
    ...
    sm_generate($originalFilepath, $resizedFilepath, $width, $height, null);
