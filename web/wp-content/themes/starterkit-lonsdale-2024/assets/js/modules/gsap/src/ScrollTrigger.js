...
let gsap,
  ...
  _rafBugFix = () => _enabled && requestAnimationFrame(_rafBugFix),
  ...
  _getSizeFunc = (scroller, isViewport, {
    ...
    d2,
    a
  }) => (a = _getProxyProp(scroller, "getBoundingClientRect")) ? () => a()[d] : () => (isViewport ? _getViewportDimension(d2) : scroller["client" + d2]) || 0,
  ...
  _maxScroll = (element, {
    ...
    d,
    a
  }) => Math.max(0, (s = "scroll" + d2) && (a = _getProxyProp(element, s)) ? a() - _getBoundsFunc(element)()[d] : _isViewport(element) ? (_docEl[s] || _body[s]) - _getViewportDimension(d2) : element[s] - element["offset" + d2]),
  ...
  _createMarker = (type, name, container, direction, {
    ...
    indent,
    fontWeight
  }, offset, matchWidthEl, containerAnimation) => {
...
export class ScrollTrigger {
  ...
  init(vars, animation) {
    ...
    if (snap) {
      ...
      _scrollers.forEach(o => _isFunction(o) && o.target === (isViewport ? _doc.scrollingElement || _docEl : scroller) && (o.smooth = false));
    ...
    self.refresh = (soft, force, position, pinOffset) => {
      ...
      if (invalidateOnRefresh && animation) {
        ...
        animation.getChildren && animation.getChildren(true, true, false).forEach(t => t.vars.immediateRender && t.render(0, true, true));
      }
      ...
      if (isFirstRefresh || prevProgress !== self.progress || containerAnimation || invalidateOnRefresh || animation && !animation._initted) {
        animation && !isToggle && (animation._initted || prevProgress || animation.vars.immediateRender !== false) && animation.totalProgress(containerAnimation && start < -0.001 && !prevProgress ? gsap.utils.normalize(start, end, 0) : prevProgress, true);
...
ScrollTrigger.getAll = () => _triggers.filter(t => t.vars.id !== "ScrollSmoother");
...
ScrollTrigger.sort = func => {
  ...
  return _triggers.sort(func || ((a, b) => (a.vars.refreshPriority || 0) * -1e6 + (a.vars.containerAnimation ? 1e6 : a._sortY) - ((b.vars.containerAnimation ? 1e6 : b._sortY) + (b.vars.refreshPriority || 0) * -1e6)));
};
